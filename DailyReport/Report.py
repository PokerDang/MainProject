...
datalatestprice = pd.DataFrame(w.wsd(index, "close", lasttradedate,lasttradedate, "Fill=Previous").Data,index=[u'最新价'],columns=name).T
...
IHlatestprice = pd.DataFrame(w.wsd(IHFuture, "close", lasttradedate,lasttradedate, "Fill=Previous").Data,index=[u'最新价'],columns=IHName).T

IFFuture = "IF1804.CFE,IF1805.CFE,IF1806.CFE,IF1809.CFE"
IFName = ['IF1804','IF1805','IF1806','IF1809']
IFlatestprice = pd.DataFrame(w.wsd(IFFuture, "close", lasttradedate,lasttradedate, "Fill=Previous").Data,index=[u'最新价'],columns=IFName).T

ICFuture = "IC1804.CFE,IC1805.CFE,IC1806.CFE,IC1809.CFE"
ICName = ['IC1804','IC1805','IC1806','IC1809']
IClatestprice = pd.DataFrame(w.wsd(ICFuture, "close", lasttradedate,lasttradedate, "Fill=Previous").Data,index=[u'最新价'],columns=ICName).T
